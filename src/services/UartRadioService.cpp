...
RadioService::Coordinate RadioService::coordinates() const {
    ...
    return Coordinate{
        map<int16_t, int8_t>(constrain(channels[0], RX_RADIO_MIN_VALUE, RX_RADIO_MAX_VALUE), RX_RADIO_MIN_VALUE , RX_RADIO_MAX_VALUE, -100, 100),
        map<int16_t, int8_t>(constrain(channels[1], RX_RADIO_MIN_VALUE, RX_RADIO_MAX_VALUE), RX_RADIO_MIN_VALUE , RX_RADIO_MAX_VALUE, -100, 100),
    };
...
void RadioService::on_interrupt(UART_HandleTypeDef* huart) {
    ...
    if (rxdata[0] != RX_RADIO_FIRST_BYTE || rxdata[RXDATA_SIZE - 1] != RX_RADIO_LAST_BYTE) {
        memset(channels, 0, CHANNELS * (sizeof(uint16_t)));
        uart.receive_it(rxdata, RXDATA_SIZE);
        return; 
    }
